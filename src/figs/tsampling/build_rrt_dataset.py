...
def get_objectives(nerf:GSplat, object_names, similarities, viz=False):
    ...
    for idx, object_name in enumerate(object_names):
        ...
        # Guard : aucun point trouvé pour cet objet
        if len(np.asarray(filtered_pcd.points)) == 0:
            print(f"[get_objectives] ⚠️  Skipping '{object_name}' — no points found.")
            continue
...
def get_objectives_old(nerf:GSplat, objectives, similarities, viz=False):
    ...
    for idx, obj in enumerate(objectives):
        ...
        if viz:
            ...
            # viz_pcd.colors=o3d.utility.Vector3dVector(np.asarray(viz_pcd.colors))
