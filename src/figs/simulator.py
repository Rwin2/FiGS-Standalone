...
#FIXME  Args:
        ...
        for i in range(Nsim):
            ...
            if i % n_sim2ctl == 0:
                ...
                # Early stopping callback (e.g. goal reached)
                if early_stop_fn is not None and early_stop_fn(xcr, k):
                    # Truncate arrays to steps actually taken
                    n_done = k + 1
                    Tro = Tro[:n_done + 1]
                    Tro[n_done] = tcr + 1.0 / hz_sim  # final time
                    Xro = Xro[:, :n_done + 1]
                    Uro = Uro[:, :n_done]
                    Tsol = Tsol[:, :n_done]
                    Adv = Adv[:, :n_done]
    ...
    def _create_solver(self):
        """Create a new ACADOS sim solver from current config (for parallel sims)."""
        drn = self.conFiG["drone"]
        uid = f"{os.getpid()}_{id(self)}_{time.time_ns()}"
        sim_json = f'figs_sim_solver_{uid}.json'
        sim_code_dir = f'c_generated_sim_{uid}'

        sim = AcadosSim()
        sim.code_export_directory = sim_code_dir
        sim.model = export_quadcopter_ode_model(drn["m"], drn["tn"])
        sim.solver_options.T = 1 / self.conFiG["rollout"]["frequency"]
        sim.solver_options.integrator_type = 'IRK'
    ...
    def simulate_parallel(self, configs, verbose=False):
        ...
        At each timestep, all N camera poses are rendered in one batched
        rasterization call.  Controllers and physics run per-sim sequentially.
        N=1 reproduces the behaviour of simulate().

        Args:
            configs:  list of dicts, each containing:
                - policy:        controller instance
                - t0, tf:        time bounds (float)
                - x0:            initial state (np.ndarray)
                - obj:           objective (np.ndarray or None)
                - query:         language query (str or None, must be same for all)
                - early_stop_fn: optional callback(xcr, step_k) -> bool
            verbose:  print per-step progress

        Returns:
            list of (Tro, Xro, Uro, Iro, Tsol, Adv) tuples, one per sim.
        """
        ...
        # All sims must share the same query (CLIP language state is global)
        query = configs[0].get("query")
        ...
        # Camera template (shared)
        camera = self.gsplat.generate_output_camera(cam_cfg)

        # Create N solvers (each has independent internal state)
        if verbose:
            print(f"Creating {N} ACADOS solvers...")
        solvers = [self._create_solver() for _ in range(N)]
        ...
        for cfg in configs:
            ...
            s.x0 = cfg["x0"]
            s.obj = cfg.get("obj")
            s.early_stop_fn = cfg.get("early_stop_fn")

            dt = np.round(s.tf - s.t0)
            n_sim2ctl = int(hz_sim / s.policy.hz)
            s.n_sim2ctl = n_sim2ctl
            s.Nsim = int(dt * hz_sim)
            s.Nctl = int(dt * s.policy.hz)
            s.n_delay = int(t_dly * hz_sim)
            s.mu_md = mu_md_s * (1 / n_sim2ctl)
            s.std_md = std_md_s * (1 / n_sim2ctl)

            # Rollout arrays
            s.Tro = np.zeros(s.Nctl + 1)
            s.Xro = np.zeros((nx, s.Nctl + 1))
            s.Uro = np.zeros((nu, s.Nctl))
            s.Iro_lists = {}
            s.Tsol = np.zeros((4, s.Nctl))
            s.Adv = np.zeros((nu, s.Nctl))
            s.Xro[:, 0] = s.x0
            ...
            s.xpr = s.x0.copy()
            s.xsn = s.x0.copy()
            s.ucm = np.array([-self.conFiG["drone"]['m'] / self.conFiG["drone"]['tn'],
                              0.0, 0.0, 0.0])
            s.udl = np.hstack((s.ucm.reshape(-1, 1), s.ucm.reshape(-1, 1)))
            s.zcr = (_torch.zeros(s.policy.nzcr)
                     if isinstance(s.policy.nzcr, int) else None)
        ...
        Wf_sn, Wf_md = Wf, 1 - Wf
        max_Nsim = max(s.Nsim for s in states)

        # --- Main loop (matches simulate() flow: Control → Physics → Store) ---
        for i in range(max_Nsim):
            # Identify active sims at a control step
            active_ctl = [j for j, s in enumerate(states)
                          if not s.done and i < s.Nsim and i % s.n_sim2ctl == 0]

            # === BLOCK A: Control (batched render + per-sim control) ===
            if active_ctl:
                T_c2w_list = []
                for j in active_ctl:
                    s = states[j]
                    Tb2w = th.xv_to_T(s.xcr)
                    T_c2w_list.append(Tb2w @ T_c2b)

                if perception == "semantic_depth" and perception_type != "clipseg" and query is not None:
                    image_dicts = self.gsplat.render_rgb_batch(camera, T_c2w_list, query)
                else:
                    image_dicts = self.gsplat.render_rgb_batch(camera, T_c2w_list)

                for idx_in_batch, j in enumerate(active_ctl):
                    s = states[j]
                    s._img = image_dicts[idx_in_batch]
                    tcr = s.t0 + i / hz_sim
                    s._tcr = tcr

                    if query is not None and "semantic" in s._img:
                        s.icr = s._img["semantic"]
                    else:
                        s.icr = s._img["rgb"]

                    if use_fusion:
                        s.xsn += np.random.normal(loc=mu_sn, scale=std_sn)
                        s.xsn = Wf_sn @ s.xsn + Wf_md @ s.xcr
                    else:
                        s.xsn = s.xcr + np.random.normal(loc=mu_sn, scale=std_sn)
                    s.xsn[6:10] = th.obedient_quaternion(s.xsn[6:10], s.xpr[6:10])

                    s.ucm, s.zcr, s._adv, s._tsol = s.policy.control(
                        tcr, s.xsn, s.ucm, s.obj, s.icr, s.zcr)
                    s.udl[:, 0] = s.udl[:, 1]
                    s.udl[:, 1] = s.ucm

            # === BLOCK B: Physics (all active sims, at sim rate) ===
            for j, s in enumerate(states):
                if s.done or i >= s.Nsim:
                    continue
                uin = s.udl[:, 0] if i % s.n_sim2ctl < s.n_delay else s.udl[:, 1]
                s.xcr = solvers[j].simulate(x=s.xcr, u=uin)
                if use_fusion:
                    s.xsn = solvers[j].simulate(x=s.xsn, u=uin)
                s.xcr = s.xcr + np.random.normal(loc=s.mu_md, scale=s.std_md)
                s.xcr[6:10] = th.obedient_quaternion(s.xcr[6:10], s.xpr[6:10])
                s.xpr = s.xcr

            # === BLOCK C: Store (at control rate, AFTER physics) ===
            for j in active_ctl:
                s = states[j]
                if s.done:
                    continue
                ...
                if s.early_stop_fn is not None and s.early_stop_fn(s.xcr, k):
                    n_done = k + 1
                    s.Tro = s.Tro[:n_done + 1]
                    s.Tro[n_done] = s._tcr + 1.0 / hz_sim
                    s.Xro = s.Xro[:, :n_done + 1]
                    s.Uro = s.Uro[:, :n_done]
